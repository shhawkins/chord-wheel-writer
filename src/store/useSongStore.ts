    ...
    persist(
        (set, get) => ({
            ...
            saveToCloud: async (song: Song) => {
                const { data: { user } } = await supabase.auth.getUser();
                if (!user) {
                    // Prompt login or error
                    return;
                }

                set({ isLoadingCloud: true });

                // prepare song data: remove id and dates from JSON blob if we want them separate, 
                // but for simplicity we store the whole object in 'data'
                // We MUST exclude the ID from the json data if we want the DB ID to be the source of truth,
                // OR we sync them. 
                // Let's use the song.id as the DB UUID if it's a valid UUID, otherwise generate one.
                // But our local IDs might not be UUIDs. 
                // Strategy: Upsert based on ID. If local ID is not UUID, valid UUID will be generated by DB (if we omit it) 
                // but then we need to update local ID.
                // Better: Check if song.id is UUID. If so, use it. If not (e.g. 'default'), generate new UUID.
                ...
                const { error } = await supabase
                    .from('songs')
                    .upsert({
                        id: song.id.length === 36 ? song.id : undefined, // Simple UUID check
                        user_id: user.id,
                        title: song.title,
                        data: song,
            ...
            saveInstrumentToCloud: async (instrument: CustomInstrument) => {
                ...
                if (error) console.error('Error saving instrument:', error);
            },

            fetchUserInstruments: async () => {
                // To be implemented fully
                return [] as CustomInstrument[];
            },
